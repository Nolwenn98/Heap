...
void test_init_heap(void)
{
    ...
    CU_ASSERT(heap->number == 127);
    CU_ASSERT(heap->next->number == FREE_ZONE);
    CU_ASSERT(libre == heap);
}

void test_get_index_block(void)
{
    Liste *heap;
    heap = init_heap();
    Liste *ptr;

    ptr = get_ptr_next_block(heap);

    // TODO: Ajouter des éléments
    // Vérifier que ça renvoit bien la bonne adresse
...
//     CU_ASSERT(first_fit(size) == heap);

//     // heap->data = 3;
//     // strcpy(&heap->data->next, "ab");
//     // heap[4] = 123;
//     // heap[5] = -1;
//     // libre = 4;
...
int main()
{
    ...
    if (
        ...
        NULL == CU_add_test(pSuite, "test of test_get_index_block()", test_get_index_block) // ||
